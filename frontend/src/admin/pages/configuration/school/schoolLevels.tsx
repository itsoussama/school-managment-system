...
import { Teacher } from "../../teachers/viewTeachers";
...
export default function SchoolLevels() {
  ...
  // const getGroupWithoutGradesQuery = useQuery({
  //   queryKey: ["getGroupWithoutGrades"],
  //   queryFn: () => getGroupWithoutGrades(admin.school_id),
  // });

  const getGroupsQuery = useQuery({
    queryKey: ["getGroups"],
    queryFn: () => getGroups(1, -1, undefined, undefined, admin.school_id),
  });
  ...
  const handleStudentList = async (gradeId: number) => {
  ...
  const handleTeacherList = async (gradeId: number) => {
            ...
            {groupData.id !== 0 && (
              <div className="mt-8 flex w-full flex-col">
                <Button
                  ...
                  onClick={() => handleStudentList(gradeData?.id as number)}
                >
                  {t("actions.view_entity", {
                    entity:
                      t("determiners.definite.plural") +
                ...
                <Button
                  ...
                  className="btn-default disabled:btn-disabled"
                  disabled={groupData.id === 0}
                  onClick={() => handleTeacherList(gradeData?.id as number)}
                >
                  {t("actions.view_entity", {
                    entity:
                      t("determiners.definite.plural") +
